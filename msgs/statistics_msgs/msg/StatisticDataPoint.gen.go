...
// StatisticDataPoint
func NewStatisticDataPointPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*StatisticDataPointPublisher, error) {
	pub, err := node.NewPublisher(topic_name, StatisticDataPointTypeSupport, options)
	if err != nil {
		return nil, err
	}
...
// StatisticDataPoint
func NewStatisticDataPointSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback StatisticDataPointSubscriptionCallback) (*StatisticDataPointSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg StatisticDataPoint
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, StatisticDataPointTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
